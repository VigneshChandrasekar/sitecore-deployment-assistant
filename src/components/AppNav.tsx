...
import { Box, Package, GitCompare, GitMerge, Shield, Terminal } from 'lucide-react'
...
export default function AppNav() {
  ...
  return (
    <header className="shrink-0 flex items-center gap-0 px-5 h-16 bg-slate-900 border-b border-slate-800">

      {/* Logo */}
      <Link href="/package-inspector" className="flex items-center gap-2.5 shrink-0 mr-6">
        ...
        <div className="leading-tight">
          ...
          <p className="text-[10px] text-slate-500 tracking-wide">Pre-deployment toolkit</p>
        </div>
      </Link>
      ...
      <nav className="flex items-stretch gap-1 flex-1">
        {MAIN_TABS.map(({ href, label, sub, icon: Icon }) => {
          ...
          return (
            ...
            >
              <Icon className={`h-4 w-4 shrink-0 ${active ? 'text-slate-700' : 'text-slate-500 group-hover:text-slate-300'}`} />
              <div className="leading-tight">
                <p className={`text-[12px] font-semibold whitespace-nowrap ${active ? 'text-slate-900' : ''}`}>
                  {label}
                </p>
                <p className={`text-[10px] whitespace-nowrap ${active ? 'text-slate-500' : 'text-slate-600 group-hover:text-slate-400'}`}>
                  {sub}
                </p>
              </div>
              {/* Active underline */}
      ...
      <div className="shrink-0 ml-4 pl-4 border-l border-slate-700">
        ...
        >
          <Terminal className={`h-3.5 w-3.5 shrink-0 ${isActive('/setup-instructions') ? 'text-white' : 'text-slate-500 group-hover:text-slate-300'}`} />
          <div className="leading-tight">
            <p className="text-[12px] font-semibold whitespace-nowrap">Setup Instructions</p>
            <p className={`text-[10px] whitespace-nowrap ${isActive('/setup-instructions') ? 'text-slate-300' : 'text-slate-600 group-hover:text-slate-400'}`}>
              PowerShell snapshot
            </p>
          </div>
        </Link>

...
export default function UploadPanel({ onParsed, pkg, loading, setLoading, refResult, onRefReport }: Props) {
  const [dragOver, setDragOver] = useState(false);
  const [error, setError]       = useState<string | null>(null);

  const process = useCallback(async (file: File) => {
    if (!file.name.endsWith(".zip")) { setError("Please upload a .zip Sitecore package file."); return; }
    setError(null);
    setLoading(true);
    try {
  ...
  const onChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0]; if (file) process(file);
  };

  const riskCount = pkg ? pkg.items.filter((i) => RISK_MODES.includes(i.deployMode)).length : 0;
          ...
          {loading ? (
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="text-left">
                <p className="text-xs font-semibold text-slate-200">Replace package</p>
                <p className="text-[11px] text-slate-500">Drop or click to browse</p>
              </div>
            </div>
          ) : (
            <div className="text-center">
              <UploadCloud className="h-6 w-6 text-slate-600 mx-auto mb-1" />
              <p className="text-xs font-medium text-slate-400">Drop a Sitecore package</p>
              <p className="text-[10px] text-slate-600">or click to browse — .zip only</p>
            </div>
          )}
        </label>
      </div>
      ...
      {pkg && !loading && (
        <>
          ...
          <Section label="Actions">
            <div className="flex flex-col gap-2">
              ...
              <div className="flex flex-col gap-2 pt-1">
                ...
                <button
                  onClick={async () => { const { exportToExcel } = await import("@/lib/exporter"); exportToExcel(pkg); }}
          ...
          <Section label="Package">
            <MetaRow icon={<Tag className="h-3 w-3" />}  label="Name"    value={pkg.metadata.name || "—"} />
            <MetaRow icon={<Hash className="h-3 w-3" />} label="Version" value={pkg.metadata.version || "—"} />
            <MetaRow icon={<User className="h-3 w-3" />} label="Author"  value={pkg.metadata.author || "—"} />
            {pkg.metadata.publisher && (
              <MetaRow icon={<User className="h-3 w-3" />} label="Publisher" value={pkg.metadata.publisher} />
            )}
          </Section>
          ...
          {riskCount > 0 && (
            <div className="mx-4 mb-1 flex items-start gap-2 rounded-lg bg-amber-500/10 border border-amber-500/20 px-3 py-2.5 text-xs text-amber-400">
              ...
              <span><strong className="text-amber-300">{riskCount}</strong> item{riskCount !== 1 ? "s" : ""} with Delete or Overwrite.</span>
            </div>
          )}
  ...
  return (
    <div className="flex items-start gap-2 py-0.5">
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="text-xs font-medium text-slate-200 break-words leading-tight">{value}</p>
      </div>
    </div>
  );
}
...
function StatCard({ label, value, danger }: { label: string; value: number; danger?: boolean }) {
  return (
    <div className={`rounded-xl px-3 py-3 text-center border ${
      danger ? "bg-red-500/15 border-red-500/30" : "bg-slate-800 border-slate-700"
    }`}>
      <p className={`text-2xl font-bold leading-none tabular-nums ${danger ? "text-red-400" : "text-white"}`}>{value}</p>
      <p className={`text-[10px] mt-1 font-medium uppercase tracking-wide ${danger ? "text-red-500" : "text-slate-500"}`}>{label}</p>
    </div>
  );
}
...
function DeployBreakdown({ pkg }: { pkg: ParsedPackage }) {
  ...
  const counts = Object.fromEntries(modes.map((m) => [m, pkg.items.filter((i) => i.deployMode === m).length])) as Record<DeployMode, number>;
        ...
        {active.map((m) => (
          <div key={m}>
            <div className="flex items-center justify-between mb-1">
              ...
              <span className="text-xs font-semibold tabular-nums text-slate-500">{counts[m]}</span>
            </div>
            <div className="h-1 bg-slate-800 rounded-full overflow-hidden">
              <div className={`h-full rounded-full transition-all ${DEPLOY_BAR_COLOR[m]}`}
...
function TypeBreakdown({ pkg }: { pkg: ParsedPackage }) {
  ...
  for (const item of pkg.items) typeCounts.set(item.itemType, (typeCounts.get(item.itemType) ?? 0) + 1);
...
function TypeDot({ type }: { type: string }) {
  const colors: Record<string, string> = {
    Template: "bg-violet-400", "Template Field": "bg-violet-300", "Template Section": "bg-purple-400",
    Rendering: "bg-blue-400", Layout: "bg-sky-400", Placeholder: "bg-cyan-400",
    Media: "bg-emerald-400", Setting: "bg-amber-400", Content: "bg-slate-400", Unknown: "bg-slate-600",
  };
  return <span className={`w-2 h-2 rounded-full shrink-0 ${colors[type] ?? "bg-slate-600"}`} />;
}
